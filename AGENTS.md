...
- **Docker build requires corporate registry**: Pass `BUN_CONFIG_REGISTRY=https://jfrog.booking.com/artifactory/api/npm/npm/` as a build arg. Without it, bun install fails with `UNKNOWN_CERTIFICATE_VERIFICATION_ERROR` inside the Docker VM.
  ```
  docker build --build-arg BUN_CONFIG_REGISTRY=https://jfrog.booking.com/artifactory/api/npm/npm/ -t pplenkov/agent-memory:latest .
  ```
